...
def get_matchmaker_prompt(scene_text: str, scene_num: str, history_summary: str) -> str:
    ...
    REFERENCE CATALOG (Items followed by the scene they last appeared in):
    {history_summary}
    ...
    --- MANDATORY LOGIC ---
    1. UNIVERSAL SPECIFICITY: Check for generic nouns. If an item exists in the REFERENCE CATALOG with more detail, create a note referencing the original scene (e.g., "Matches Sc 1 Duffel Bags").
    2. STRICT MATCHING: Only map items if they are the same type of object. NEVER match unrelated items.
    ...
    4. THE "NO-PEOPLE" RULE: Strictly ignore all Characters/People.
    5. SCOPE: Apply this to all physical production categories in the Catalog.
    6. NO REASONING/ADVICE: Keep notes short and technical (e.g., "Use Scene 1 Duffel Bags").
